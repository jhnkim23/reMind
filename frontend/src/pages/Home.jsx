...
function Home() {
    ...
    async function uploadFile(file, type) {
      ...
      try {
        ...
        const response = await axios.post('http://127.0.0.1:10000/api/upload_transcript/', formData, {
            headers: {
    ...
    async function uploadAudio(file, type) {
      ...
      try {
        ...
        const response = await axios.post('http://127.0.0.1:10000/api/create-transcript/', formData, {
            headers: {
                "Content-Type": "multipart/form-data"
            }
        });
    ...
    async function handleClick(e) {
      ...
      if (e.target.id === 't') {
        if (transcriptFile) {
          ...
          if (audioFile) {
              await uploadAudio(audioFile, "input_file");
          } else {
